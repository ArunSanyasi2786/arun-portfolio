...
export default function SectionHeading({ eyebrow, title, children, align = 'left' }) {
  return (
    ...
    >
      <p className="mb-3 font-mono text-xs font-bold uppercase tracking-[0.35em] text-cyan-300">{eyebrow}</p>
      <h2 className="font-display text-3xl font-bold tracking-tight text-white sm:text-4xl lg:text-5xl">{title}</h2>
      {children ? <p className="mt-4 text-base leading-8 text-slate-300 sm:text-lg">{children}</p> : null}
    </motion.div>
  );
}
