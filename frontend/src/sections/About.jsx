    ...
    <section id="about" className="relative px-4 py-24 sm:px-6 lg:px-8" aria-labelledby="about-title">
      <div className="mx-auto max-w-7xl">
          ...
          >
            ...
            <div className="mt-6 space-y-5 text-base leading-8 text-slate-300">
              {aboutHighlights.map((item) => (
                <p key={item} className="border-l border-cyan-300/30 pl-4 transition hover:border-cyan-200 hover:text-white">{item}</p>
              ))}
            </div>
          </motion.div>
          <div className="grid gap-4 sm:grid-cols-2">
            {pillars.map((pillar, index) => {
              ...
              return (
                ...
                >
                  <Icon className="h-8 w-8 text-cyan-300 transition group-hover:scale-110 group-hover:text-white" />
                  <h4 className="mt-4 font-display text-xl font-bold text-white">{pillar.title}</h4>
                  <p className="mt-3 text-sm leading-7 text-slate-400 group-hover:text-slate-300">{pillar.text}</p>
                </motion.article>
              );
            })}
          </div>
        ...
        <motion.div
          ...
          className="mt-6 glass-card rounded-[2rem] p-6 sm:p-8"
        >
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="font-mono text-[11px] font-bold uppercase tracking-[0.25em] text-cyan-300">Academic Foundation</p>
              <h3 className="mt-2 font-display text-2xl font-bold text-white">Education</h3>
            </div>
            <div className="inline-flex w-fit items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/[0.06] px-4 py-2 text-xs font-bold text-cyan-100">
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-3">
            {education.map((item, index) => (
              <motion.article
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.08 }}
                className="group relative overflow-hidden rounded-3xl border border-cyan-300/10 bg-cyan-300/[0.04] p-5"
              >
                <div className="absolute -right-8 -top-8 h-24 w-24 rounded-full bg-cyan-300/10 blur-2xl transition group-hover:bg-cyan-300/25" />
                <p className="font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-cyan-300">{item.period}</p>
                <h4 className="mt-3 font-display text-lg font-bold text-white">{item.degree}</h4>
                <p className="mt-2 text-sm font-semibold text-slate-300">{item.institution}</p>
                <p className="mt-1 text-xs text-slate-500">{item.location}</p>
                <p className="mt-4 inline-flex rounded-full border border-cyan-300/20 bg-slate-950/50 px-3 py-1 text-xs font-bold text-cyan-100">{item.result}</p>
                <p className="mt-4 text-sm leading-7 text-slate-400">{item.focus}</p>
                <div className="mt-4 flex flex-wrap gap-2">
                  {item.highlights.map((highlight) => (
                    <span key={highlight} className="rounded-full bg-cyan-300/10 px-3 py-1 text-[11px] font-semibold text-cyan-100">{highlight}</span>
                  ))}
                </div>
              </motion.article>
            ))}
